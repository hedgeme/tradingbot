...
def compute_slippage(token_in: str, token_out: str, amount_in: Decimal, default_bps: int = 30) -> Optional[dict]:
    ...
    if out_wei is None:
        if _find_pool(token_in, "WONE") and _find_pool("WONE", token_out):
    ...
    impact_bps = Decimal(0) if implied_no_impact == 0 else (1 - (amount_out / implied_no_impact)) * Decimal(10000)
